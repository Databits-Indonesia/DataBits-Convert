...
// Package versions for WASM modules
...
export function getWasmBaseUrl(packageName: WasmPackage): string | undefined {
  ...
  if (userUrl) {
    console.log(`[WASM Config] Using configured URL for ${packageName}: ${userUrl}`);
    return userUrl;
  }

  console.warn(`[WASM Config] No URL configured for ${packageName}. Feature unavailable.`);
  return undefined;
}
...
export async function fetchWasmFile(packageName: WasmPackage, fileName: string): Promise<Response> {
  const baseUrl = getWasmBaseUrl(packageName);

  if (!baseUrl) {
    throw new Error(`No URL configured for ${packageName}. Please configure it in WASM Settings.`);
  }
