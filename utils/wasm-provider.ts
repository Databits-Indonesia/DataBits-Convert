...
class WasmProviderManager {
  ...
  private loadConfig(): WasmProviderConfig {
    ...
    try {
      ...
      console.warn('[WasmProvider] Failed to load config from localStorage:', e);
    }
    return {};
  }
  ...
  hasAnyProvider(): boolean {
    return Object.keys(this.config).length > 0 || Object.values(ENV_DEFAULTS).some(Boolean);
  }
  ...
  async validateUrl(
    ...
    try {
      ...
      return {
        valid: false,
        error: 'Invalid URL format. Please enter a valid URL (e.g., https://example.com/wasm/)',
      };
    }
...
export function showWasmRequiredDialog(packageName: WasmPackage, onConfigure?: () => void): void {
  ...
  modal.className = 'bg-gray-800 rounded-2xl max-w-md w-full shadow-2xl border border-gray-700';
  ...
  configureBtn?.addEventListener('click', () => {
    ...
    if (onConfigure) {
      ...
      window.location.href = `/wasm-settings.html`;
    }
  });
}

export function requireWasm(packageName: WasmPackage, onAvailable?: () => void): boolean {
