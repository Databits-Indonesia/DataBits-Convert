...
export async function loadPyMuPDF(): Promise<any> {
  ...
  loadPromise = (async () => {
    if (!WasmProvider.isConfigured('pymupdf')) {
      throw new Error('PyMuPDF is not configured. Please configure it in Advanced Settings.');
    }
    ...
    const normalizedPymupdf = pymupdfUrl.endsWith('/') ? pymupdfUrl : `${pymupdfUrl}/`;

    try {
      const wrapperUrl = `${normalizedPymupdf}dist/index.js`;
      
      // Use dynamic import for ES module loading
      const module: any = await import(/* webpackIgnore: true */ wrapperUrl);
...
export function isPyMuPDFAvailable(): boolean {
  return WasmProvider.isConfigured('pymupdf') && WasmProvider.isConfigured('ghostscript');
}
