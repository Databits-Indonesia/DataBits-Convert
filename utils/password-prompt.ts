...
function validatePasswordWithPdfjs(pdfBytes: ArrayBuffer, password: string): Promise<boolean> {
...
async function decryptFileWithPassword(file: File, password: string): Promise<File> {
  ...
  const modalPromise = new Promise<File | null>((resolve) => {
    ...
    async function onSubmit() {
      ...
      try {
        ...
        if (progressEl) progressEl.classList.add('hidden');
        if (errorEl) {
          errorEl.textContent = 'Failed to decrypt. Try the Decrypt tool instead.';
          errorEl.classList.remove('hidden');
        }
      }
    }
...
export async function promptAndDecryptBatch(
  ...
  if (cachedPassword) {
    ...
    if (allValid && cachedPassword) {
      ...
      for (const idx of encryptedIndices) {
        try {
          tempMap.set(idx, await decryptFileWithPassword(files[idx], cachedPassword));
        } catch {
          cachedPassword = null;
          allDecrypted = false;
          break;
  ...
  if (!submitBtn || !cancelBtn || !samePwCheckbox || !sharedInput || !filelistEl) {
    return decryptedFiles;
  }

  if (titleEl) titleEl.textContent = `${fileNames.length} Files Need a Password`;
  ...
  const batchPromise = new Promise<Map<number, File>>((resolve) => {
    ...
    function markRowSuccess(localIdx: number) {
      ...
      const row = filelistEl.querySelector<HTMLDivElement>(`[data-file-idx="${localIdx}"]`);
      if (!row) return;
      ...
      const iconEl = row.querySelector<HTMLElement>(`[data-icon-idx="${localIdx}"]`);
      if (iconEl) {
    ...
    function onSkipFile(e: Event) {
      if (busy) return;
      const btn = (e.target as HTMLElement).closest<HTMLButtonElement>('[data-skip-idx]');
      ...
      const row = filelistEl.querySelector<HTMLDivElement>(`[data-file-idx="${idx}"]`);
      if (!row) return;
    ...
    function cleanup() {
      ...
      if (sharedToggle) sharedToggle.removeEventListener('click', onSharedToggle);
    ...
    async function onSubmit() {
      ...
      for (let i = 0; i < fileNames.length; i++) {
        ...
        if (useSame) {
          ...
          const pwInput = filelistEl.querySelector<HTMLInputElement>(`input[data-pw-idx="${i}"]`);
      ...
      for (let i = 0; i < toProcess.length; i++) {
        ...
        const bytes = (await readFileAsArrayBuffer(files[realIdx])) as ArrayBuffer;
        ...
        try {
          const decrypted = await decryptFileWithPassword(files[realIdx], password);
...
export async function batchDecryptIfNeeded(files: File[]): Promise<File[]> {
  ...
  const skippedSet = new Set(encryptedIndices.filter((idx) => !decryptedMap.has(idx)));
