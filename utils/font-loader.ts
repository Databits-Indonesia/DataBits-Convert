...
type OcrFontEnv = {
  VITE_OCR_FONT_BASE_URL?: string;
};

function getDefaultFontEnv(): OcrFontEnv {
  return {
    VITE_OCR_FONT_BASE_URL:
      process.env.NEXT_PUBLIC_OCR_FONT_BASE_URL || process.env.VITE_OCR_FONT_BASE_URL,
  };
}
...
export function resolveFontUrl(fontFamily: string, env: OcrFontEnv = getDefaultFontEnv()): string {
