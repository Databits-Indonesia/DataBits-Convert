...
export async function pdfToText() {
  ...
  try {
    ...
    let fullText = '';
    const includePageNumbers = (document.getElementById('pdf-to-text-page-numbers') as HTMLInputElement)?.checked || false;
    const separator = (document.getElementById('pdf-to-text-separator') as HTMLSelectElement)?.value || 'double-line';
    ...
    showAlert(
      'Error',
      `An error occurred during text extraction. ${error.message}`
    );
  }
}
