  ...
  try {
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      if (totalRotation % 90 === 0) {
        ...
        const x =
          newWidth / 2 - ((width / 2) * Math.cos(angleRad) - (height / 2) * Math.sin(angleRad));
        const y =
          newHeight / 2 - ((width / 2) * Math.sin(angleRad) + (height / 2) * Math.cos(angleRad));
...
async function renderThumbnails() {
  ...
  await renderPagesProgressively(pageState.pdfJsDoc, pageThumbnails, createPageWrapper, {
    batchSize: 8,
    useLazyLoading: true,
