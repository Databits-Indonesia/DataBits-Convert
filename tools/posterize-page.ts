...
function drawGridOverlay() {
  ...
  if (pagesToProcess.includes(pageState.currentPage - 1)) {
    const rows =
      parseInt((document.getElementById('posterize-rows') as HTMLInputElement).value) || 1;
    const cols =
      parseInt((document.getElementById('posterize-cols') as HTMLInputElement).value) || 1;
...
async function posterize() {
  ...
  try {
    ...
    const cols =
      parseInt((document.getElementById('posterize-cols') as HTMLInputElement).value) || 1;
    const pageSizeKey = (document.getElementById('output-page-size') as HTMLSelectElement)
      .value as keyof typeof PageSizes;
    const orientation = (document.getElementById('output-orientation') as HTMLSelectElement).value;
    const scalingMode = (
      document.querySelector('input[name="scaling-mode"]:checked') as HTMLInputElement
    ).value;
    ...
    for (const pageIndex of pageIndicesToProcess) {
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          ...
          const sy = r * tileHeight - (r > 0 ? overlapInPoints : 0);
          const sWidth =
            tileWidth + (c > 0 ? overlapInPoints : 0) + (c < cols - 1 ? overlapInPoints : 0);
          const sHeight =
            tileHeight + (r > 0 ? overlapInPoints : 0) + (r < rows - 1 ? overlapInPoints : 0);
          ...
          if (tileCtx) {
            ...
            const scaleY = newPage.getHeight() / sHeight;
            const scale =
              scalingMode === 'fit' ? Math.min(scaleX, scaleY) : Math.max(scaleX, scaleY);
