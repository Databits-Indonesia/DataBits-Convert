...
function getWorker() {
  if (!worker) {
    try {
      worker = new Worker('/workers/edit-attachments.worker.js');
...
async function loadAttachmentsList() {
  ...
  try {
    ...
    if (!w) {
      hideLoader();
      showAlert('Error', 'This feature requires Web Workers which are not available.');
      return;
    }
    w.postMessage(message, [fileBuffer]);
  } catch (error) {
