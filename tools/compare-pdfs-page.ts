...
function updateNavControls() {
  const maxPages = Math.max(pageState.pdfDoc1?.numPages || 0, pageState.pdfDoc2?.numPages || 0);
...
function setViewMode(mode: 'overlay' | 'side-by-side') {
  ...
  if (mode === 'overlay') {
    if (wrapper)
      wrapper.className =
        'compare-viewer-wrapper overlay-mode bg-gray-900 rounded-lg border border-gray-700 min-h-[400px] relative';
    ...
    if (wrapper)
      wrapper.className =
        'compare-viewer-wrapper side-by-side-mode bg-gray-900 rounded-lg border border-gray-700 min-h-[400px]';
